...
export async function getOrCreateConversation(userId: string, userName: string, userAvatar?: string): Promise<{ conversation: any; isNew: boolean }> {
  const client = await pool.connect();
  try {
...
export async function getProjectDescription(repoName: string): Promise<{
  description: string;
  ...
  try {
    const result = await client.query(
      'SELECT description, is_auto_generated, source, regenerate_flag FROM project_descriptions WHERE repo_name = $1',
      [repoName]
    );
...
export async function saveProjectDescription(
  ...
  try {
    await client.query(
      `INSERT INTO project_descriptions (repo_name, description, is_auto_generated, source, regenerate_flag, updated_at)
      ...
      [repoName, description, isAutoGenerated, source]
    );
  } finally {
    client.release();
  }
...
export async function markForRegeneration(repoName: string): Promise<void> {
  const client = await pool.connect();
  try {
    await client.query(
      `INSERT INTO project_descriptions (repo_name, description, regenerate_flag, updated_at)
       VALUES ($1, '', TRUE, CURRENT_TIMESTAMP)
       ON CONFLICT (repo_name)
       DO UPDATE SET regenerate_flag = TRUE, updated_at = CURRENT_TIMESTAMP`,
      [repoName]
    );
  } finally {
    client.release();
